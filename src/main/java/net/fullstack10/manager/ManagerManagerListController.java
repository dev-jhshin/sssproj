...
/**
 * Servlet implementation class ManagerManagerListController
 */
@WebServlet("/manager/managerList.do")
public class ManagerManagerListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		total_count = dao.getManagerTotalCount(mMap);
		List<ManagerDTO> managerList = dao.getManagerList(mMap);
		dao.close();
		
		mMap.put("totalManager", total_count);
		mMap.put("managerList", managerList);
		...
		mMap.put("paging", 
				managerPage.managerPagingArea(total_count, 
					...
					"./managerList.do?"+queryString)
		);
		request.getRequestDispatcher("/WEB-INF/views/manager/admin_list_page.jsp").forward(request, response);
	}
