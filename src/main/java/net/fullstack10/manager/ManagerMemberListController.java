...
import net.fullstack10.auth.AuthDAO;
import net.fullstack10.auth.AuthDTO;
import net.fullstack10.bbs.BbsDTO;
...
public class ManagerMemberListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		total_count = Adao.getMemberTotalCount(mMap);
		List<AuthDTO> memberList = Adao.getMemberList(mMap);
		Adao.close();

		mMap.put("totalMember", total_count);
		...
		mMap.put("paging", 
				managerPage.managerPagingArea(total_count, 
					...
					"./memberList.do?"+queryString)
		);
		request.getRequestDispatcher("/WEB-INF/views/manager/adStuListPage.jsp").forward(request, response);
