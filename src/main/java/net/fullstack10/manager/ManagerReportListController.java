...
/**
 * Servlet implementation class ManagerReportListController
 */
@WebServlet("/manager/reportList.do")
public class ManagerReportListController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		total_count = dao.getReportTotalCount(mMap);
		List<ManagerDTO> reportList = dao.getReportList(mMap);
		dao.close();

		mMap.put("totalReport", total_count);
		mMap.put("reportList", reportList);
		...
		mMap.put("paging",
				managerPage.managerPagingArea(total_count,
					...
					"./reportList.do?"+queryString)
		);
		request.getRequestDispatcher("/WEB-INF/views/manager/admin_report_list_page.jsp").forward(request, response);
	}
